...
def userLib(*args):
    from plt_maya import UserLibrary
    reload(UserLibrary)
    UserLibrary.initialize()


def tweenerUI(*args):
    from plt_maya.modules.Animation import Tweener
    reload(Tweener)
    Tweener.initialize()


def fixTexturePathUI(*args):
    from plt_maya.modules import FixPath
    reload(FixPath)
    FixPath.initialize()
...
def customViewer(*args):
    from plt_maya.modules import CustomViewer
    reload(CustomViewer)
...
def layerManagerUI(*args):
    from plt_maya.modules import LayerManager
    reload(LayerManager)
...
def reloadDataMainUI(*args):
    from plt_maya import InitTool
    reload(InitTool)
    InitTool.initilize()


def refreshMainUI(*args):
    from plt_maya.modules import MayaMainUI
    reload(MayaMainUI)
    MayaMainUI.MayaMainUI()


def newProd(*args):
    from plt_maya.modules import ProdFolder
    reload(ProdFolder)
    ProdFolder.ProdFolder()


def lightManager(*args):
    from plt_maya.modules import ToolBoxIII
    reload(ToolBoxIII)
    ToolBoxIII.showUI()


def channelBoxUI(*args):
    from plt_maya.modules import ChannelBox
    reload(ChannelBox)
    ChannelBox.ChannelBox()


def toolBoxI(*args):
    from plt_maya.modules import ToolBoxI
    reload(ToolBoxI)
    ToolBoxI.DAMGtoolBoxI()


def createGear(*args):
    from plt_maya.modules.Modeling import gearCreator
    reload(gearCreator)
    gearCreator.gear()


def toolBoxII(*args):
    from plt_maya.modules import ToolBoxII
    reload(ToolBoxII)
    ToolBoxII.ToolBoxII()
...
def toolBoxIV(*args):
    from plt_maya.modules import ToolBoxIV
    reload(ToolBoxIV)
    ToolBoxIV.toolBoxIV()
...
def publishUI(mode, *args):
    if mode == 'Studio Mode':
        from plt_maya.modules import DataHandle_studio
        reload(DataHandle_studio)
        DataHandle_studio.DataHandle().publishUI()
    elif mode == 'Group Mode':
        from plt_maya.modules import DataHandle_group
...
def projManagerUI(*args):
    from plt_maya.modules import ProjectManager
