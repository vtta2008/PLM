...
class StylesheetManager(object):
            ...
            for path in spaths:
                if path not in cfg_paths:
                    if not os.path.exists(path):
                        logger.warning('config path "%s" does not exist, skipping.' % path)
                        continue
                    logger.debug('reading config external path: "%s".' % path)
        ...
        if 'SCENEGRAPH_STYLESHEET_PATH' in os.environ:
            ...
            for path in qpaths:
                if path not in qss_paths:
                    if not os.path.exists(path):
                        logger.warning('stylesheet path "%s" does not exist, skipping.' % path)
                        continue
                    logger.debug('reading external stylesheet path: "%s".' % path)
        ...
        for path in paths:
            for fn in os.listdir(path):
                ...
                if fext.lower() in ['.ini', '.cfg']:
                    ...
                    names = bn.split('-')
                    if len(names) < 2:
                        logger.warning('improperly named config file: "%s"' % cfg_file)
                        continue
                    ...
                    logger.debug('adding %s config "%s" from "%s".' % (cfg_type, style_name, cfg_file))
                ...
                if fext.lower() in ['.qss', '.css']:
                    ...
                    if qss_file not in qss_files.values():
                        ...
                        if style_name is None:
                            logger.warning('cannot parse style name from "%s".' % qss_file)
                            style_name = 'no-style'

                        logger.debug('adding stylesheet "%s" from "%s".' % (style_name, qss_file))
    ...
    def font_defaults(self, platform=None, style='default'):
        ...
        if not os.path.exists(def_font_config):
            logger.error('config "%s" does not exist.' % def_font_config)
            return defaults
...
class StyleParser(object):
    ...
    def apply(self, stylesheet=None, style=None):
        ...
        * Not used.        
        ...
        from ui import stylesheet as default_stylesheet
        ssf = QFile(default_stylesheet)
        ssf.open(QFile.ReadOnly)
        self.ui.setStyleSheet(str(ssf.readAll()))
