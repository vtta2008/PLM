...
from appData.core.metadata import MetadataParser
from ui.options import SCENEGRAPH_STYLESHEET_PATH, PLATFORM
...
class AttributeEditor(QWidget):
    ...
    def buildLayout(self, verbose=False):
        ...
        for section in self._sections:            
            ...
            group = QGroupBox(self.mainGroup)
            ...
            for attr_name, attr_attrs in attrs.iteritems():
                ...
                if not private or self._show_private:
                    ...
                    if 'attr_type' not in attr_attrs:
                        logger.debug('attribute "{0}.{1} has no type.'.format(self._nodes[0].name, attr_name))
                        continue

                    attr_type = attr_attrs.get('attr_type')
                    logger.info('building {0} attribute: "{1}"'.format(attr_type, attr_name))
                    ...
                    if attr_type is not None:
                        ...
                        if editor:
                            if editor:
                                ...
                                formLayout.setAlignment(label, Qt.AlignVCenter)
    ...
    def createContextMenu(self):
        ...
        popup_menu.clear()
        qcurs = QCursor()
        add_action = QAction('Add attribute', self)
                ...
                if attribute.user:
                    ...
                    for pname, pval in attribute.data.iteritems():
                        if pname not in attributes.get(name):
                            logger.debug('adding user property: "%s": %s' % (pname, pval))
...
class AddAttributeDialog(QDialog):
    ...
    def __init__(self, parent=None, nodes=[]):
        ...
        self.groupBoxLayout.setFieldGrowthPolicy(QFormLayout.AllNonFixedFieldsGrow)
        self.groupBoxLayout.setFormAlignment(Qt.AlignCenter)
        ...
        self.nameLabel.setMinimumSize(QSize(110, 0))
        self.nameLabel.setObjectName("nameLabel")
        self.groupBoxLayout.setWidget(0, QFormLayout.LabelRole, self.nameLabel)
        self.nameLineEdit = QLineEdit(self.groupBox)
        self.nameLineEdit.setObjectName("nameLineEdit")
        self.groupBoxLayout.setWidget(0, QFormLayout.FieldRole, self.nameLineEdit)
        
        # id (nyi)
        self.iDLabel = QLabel(self.groupBox)
        self.iDLabel.setMinimumSize(QSize(110, 0))
        self.iDLabel.setObjectName("iDLabel")
        self.groupBoxLayout.setWidget(1, QFormLayout.LabelRole, self.iDLabel)
        self.iDLineEdit = QLineEdit(self.groupBox)
        self.iDLineEdit.setObjectName("iDLineEdit")
        self.groupBoxLayout.setWidget(1, QFormLayout.FieldRole, self.iDLineEdit)
        
        # attribute type
        self.typeLabel = QLabel(self.groupBox)
        self.typeLabel.setMinimumSize(QSize(110, 0))
        self.typeLabel.setObjectName("typeLabel")
        self.groupBoxLayout.setWidget(2, QFormLayout.LabelRole, self.typeLabel)
        self.typeComboBox = QComboBox(self.groupBox)
        self.typeComboBox.setObjectName("typeComboBox")
        self.groupBoxLayout.setWidget(2, QFormLayout.FieldRole, self.typeComboBox)
        self.checkbox_connectable = QCheckBox(self.groupBox)
        self.checkbox_connectable.setObjectName("checkbox_connectable")
        self.groupBoxLayout.setWidget(3, QFormLayout.FieldRole, self.checkbox_connectable)
        
        # connection type
        self.connectionTypeLabel = QLabel(self.groupBox)
        self.connectionTypeLabel.setMinimumSize(QSize(110, 0))
        self.connectionTypeLabel.setObjectName("connectionTypeLabel")
        self.groupBoxLayout.setWidget(4, QFormLayout.LabelRole, self.connectionTypeLabel)
        self.connectionTypeMenu = QComboBox(self.groupBox)
        self.connectionTypeMenu.setObjectName("connectionTypeMenu")
        self.groupBoxLayout.setWidget(4, QFormLayout.FieldRole, self.connectionTypeMenu)
    ...
    def acceptedAction(self):
        ...
        if attr_name:
            for node in self._nodes:
                logger.info('adding "%s" to node "%s" (%s)' % (attr_name, node.name, attr_type))
    ...
    def sizeHint(self):
        return QSize(315, 190)

# -sub widgets ---

class QFloatEditor(QWidget):
