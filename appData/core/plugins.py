...
from ui.options import SCENEGRAPH_PATH, SCENEGRAPH_CORE, SCENEGRAPH_PLUGIN_PATH, SCENEGRAPH_ICON_PATH, SCENEGRAPH_METADATA_PATH
...
class PluginManager(object):
    ...
    def load_widgets(self, path=None, plugins=[]):
        logger.info('loading plugin widgets...')
    ...
    def get_dagnode(self, node_type, **kwargs):
        if node_type not in self._node_data:
            logger.error('plugin type "%s" is not loaded.' % node_type)
            return
    ...
    def get_widget(self, dagnode, **kwargs):
        if dagnode.node_type not in self._node_data:
            logger.error('plugin "%s" is not loaded.' % dagnode.node_type)
            return

        if 'widget' not in self._node_data.get(dagnode.node_type):
            logger.error('plugin "%s" widget not loaded.' % dagnode.node_type)
            return
    ...
    def enable(self, plugin, enabled=True):
        if not plugin in self._node_data:
            logger.error('plugin "%s" not recognized.' % plugin)
            return False

        for plug, plugin_attrs in self._node_data.iteritems():
            if plug == plugin:
                logger.info('setting plugin "%s" enabled: %s' % (plugin, str(enabled)))
    ...
    def flush(self):
        ...
        if flush:
            for f in flush:
                globals().pop(f)
                logger.info('flushing object: "%s"'% f)
...
def get_modules(path):
    ...
    for loader, mod_name, ispkg in modules:
        ...
        logger.debug('reading module: "%s"' % mod_name)
