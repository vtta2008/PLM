...
from ui import options, PluginManager
from appData.core import Attribute, EventHandler, nodes
...
class Graph(object):
    ...
    def scanNodeTypes(self, path):
        ...
        if not os.path.exists(path):
            logger.warning('node path "%s" does not exist.' % path)
            return nodes
            ...
            if fn not in ['README']:
                ...
                if os.path.exists(node_mod) and os.path.exists(node_data):
                    ...
                    if not os.path.exists(node_mod):
                        logger.warning('cannot find "%s" module %s' % (node_name, node_mod))
                    if not os.path.exists(node_data):
                        logger.warning('cannot find "%s" metadata %s' % (node_name, node_data))
        return nodes
    ...
    def updateDagNodes(self, dagnodes, debug=False):
        ...
        for dag in dagnodes:
            logger.debug('Graph: updating dag node "%s"' % dag.name)
            nid = dag.id
            if nid in self.network.nodes():
        ...
        if self.network.nodes():
            for node in self.network.nodes_iter(data=True):
                ...
                if node_id not in node_ids:
                    ...
                    logger.warning('invalid NetworkX node "%s" ( %s )' % (node_attrs.get('name'), node_id))
                    result = False
        return result
    ...
    def setScene(self, filename=None):
        ...
        if not tmp_dir:
            logger.warning('environment "TMPDIR" not set, please set and restart.')
    ...
    def add_edge(self, src, dest, **kwargs):
        ...
        if src is None or dest is None:
            logger.warning('none type passed.')
            return False

        if not src or not dest:
            logger.warning('please specify two nodes to connect.')
            return False

        # don't connect the same node
        if src.name == dest.name:
            logger.warning('invalid connection: "%s", "%s"' % (src.name, dest.name))
            return

        conn_str = '%s.%s,%s.%s' % (src.name, src_attr, dest.name, dest_attr)
        if conn_str in self.connections():
            logger.warning('connection already exists: %s' % conn_str)
            return 
        ...
        logger.info('adding edge: "%s"' % self.edge_nice_name(src.id, dest.id))
    ...
    def get_edge(self, *args):
        ...
        if not src_conn or not dest_conn:
            logger.warning('invalid arguments passed.')
            return
    ...
    def remove_edge(self, *args): 
        ...
        edges = self.get_edge(*args)
        if not edges:
            logger.warning('cannot find edge.')
            return
        ...
        for edge in edges:
            ...
            if edge_id in self.network.edges():
                logger.debug('Removing edge: "%s"' % self.edge_nice_name(*edge_id))
    ...
    def renameNode(self, old_name, new_name):
        ...
        """
        if not self.is_valid_name(new_name):
            logger.error('"%s" is not unique' % new_name)
            return
                ...
                for edge in self.network.edges(data=True):
                    ...
                    if id in [src_id, dest_id]:
                        for attr in ['src_attr', 'dest_attr']:
                            ...
                            if val is not None:
                                if val == old:
                                    print('updating attribute name: "{0}": "{1}" ("{2}")'.format(attr, new, old))
    ...
    def pasteNodes(self, nodes, offset=[200, 200]):
        ...
        for node in nodes:
            ...
            print('# adding node: "{0}"'.format(new_node.name))
    ...
    def snapshot(self):
        ...
        if not self.evaluate():
            logger.warning('graph did not evaluate correctly.')
    ...
    def read(self, filename, force=False):
        ...
        if not graph_data:
            logger.error('scene "%s" appears to be invalid.' % filename)
            return False
        ...
        if len(file_data) > 1:
            ...
            if api_ver:
                ...
                if not self.version_check(graph_data):
                    if not force:
                        logger.error('scene "%s" requires api version %s ( %s )' % (filename, options.API_MINIMUM, api_ver[0]))
                        return False   
    ...
    def read_file(self, filename):
        ...
        if os.path.exists(autosave_file):
            ...
            logger.info('removing autosave "%s"' % autosave_file)

        logger.info('reading scene file "%s"' % filename)
    ...
    def restore(self, data, nodes=True, graph=True):
        ...
        if nodes:
            for node_attrs in node_data:
                ...
                logger.debug('building node "%s"' % node_attrs.get('name'))
            ...
            for edge in edge_data:
                ...
                if not src_dag_nodes or not dest_dag_nodes:
                    logger.warning('cannot parse nodes.')
                    return
                ...
                logger.info('connecting nodes: "%s" "%s"' % (src_string, dest_string))
    ...
    def plugins(self):
        ...
        if self.plug_mgr._node_data:
            print('-' * 35)
            print('PLUGINS LOADED: {0}'.format(len(self.plug_mgr._node_data)))
        ...
        for node_type, data in self.plug_mgr._node_data.iteritems():
            ...
            if not row:
                print('{0}\nPlugin: {1}\n{2}'.format('-' *35, node_type, '-' * 35))
            else:
                print('\n{0}\nPlugin: {1}\n{2}'.format('-' *35, node_type, '-' * 35))
            print('source file: {0}'.format(data.get('source')))
            print('metadata: {0}'.format(data.get('metadata')))
            print('class: {0}'.format(data.get('class')))
            print('category:{0}'.format(data.get('category')))
            print('dagnode{0}'.format(dagnode))
