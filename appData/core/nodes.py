...
from appData.core import Attribute, EventHandler, MetadataParser
from ui.options import SCENEGRAPH_PATH, SCENEGRAPH_CORE, SCENEGRAPH_PLUGIN_PATH, SCENEGRAPH_METADATA_PATH
...
class Node(object):
    ...
    def __init__(self, name=None, **kwargs):
        ...
        if attributes:
            print('# DEBUG: {0} attributes: '.format(self.Class(), attributes))
    ...
    def __setattr__(self, name, value):
        if name in ['_attributes', '_changed', '_widget', '_metadata', 'nodeNameChanged', 'nodePositionChanged', 'nodeAttributeUpdated']:
    ...
    def read_metadata(self, verbose=False):
        ...
        for cls in reversed(result):
            ...
            if verbose:
                print('- base class "{0}" source file: "{1}"'.format(cname, py_src))
            ...
            metadata_filename = os.path.join(dirname, '{0}.mtd'.format(basename))

            # look for code node metadata in ../mtd
            if py_src == sg_core_path:
                if node_type:             
                    metadata_filename = os.path.join(SCENEGRAPH_METADATA_PATH, '{0}.mtd'.format(node_type))
                    if verbose:
                        print('- metadata file for "{0}": "{1}"' % (cname, metadata_filename))

            if not os.path.exists(metadata_filename):
                if not verbose:
                    logger.warning('plugin description file "{0}" does not exist.'.format(metadata_filename))
                else:
                    print('WARNING: metadata file for "{0}": "{1}" not found'.format(cname, metadata_filename))
                continue

            logger.debug('reading plugin metadata file: "{0}".'.format(metadata_filename))
            # parse the metadata 
...
class DagNode(Node):
    ...
    def buildConnections(self, verbose=False):
        ...
        if verbose:
            print('\n# {0}:"{1}":'.format(self.Class(), self.name))
        ...
        for section in self._metadata.data:
            ...
            for attr_name in attributes:                
                ...
                if verbose:
                    print('"{0}: {1}"'.format(attr_label, attr_name))
                    print('"attr_type": "{0}"'.format(attr_type))
