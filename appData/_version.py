...
def git_pieces_from_vcs(tag_prefix, root, verbose, run_command=run_command):
    ...
    describe_out = run_command(GITS, ["describe", "--tags", "--dirty", "--always", "--long"], cwd = root) # --long was added in git-1.5.5
    ...
    if "-" in git_describe:
        ...
        if not full_tag.startswith(tag_prefix):
            if verbose:
                ...
                pieces["error"] = ("tag '{full_tag}' doesn't start with prefix '{tag_prefix}'".
                                   format(full_tag=full_tag, tag_prefix=tag_prefix))
                return pieces

        pieces["closest-tag"] = full_tag[len(tag_prefix):]
        pieces["distance"] = int(mo.group(2))   # distance: number of commits since tag
        pieces["short"] = mo.group(3)   # commit: short hex revision ID
    else:
        # HEX: no tags
...
def render_pep440(pieces):
    ...
    if pieces["closest-tag"]:
        ...
        # exception #1
            rendered = "0+untagged.{distance}d.g{short}".format(distance=pieces["distance"], short=pieces["short"])
            if pieces["dirty"]:
                rendered += ".dirty"
            return rendered
