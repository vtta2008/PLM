...
"""
# -------------------------------------------------------------------------------------------------------------
import argparse, glob, io, itertools, os, re, sre_constants, sys, warnings
...
RE_DETECT_SECTION_TYPE = re.compile(r"^bumpversion:"
...
def main(original_args=None):
    ...
    context = dict(itertools.chain(time_context.items(), prefixed_environ().items(), vcs_info.items(), special_char_context.items(), ) )
    ...
    files.extend(ConfiguredFile(file_name, version_config) for file_name in (file_names or positionals[1:]))
...
def _parse_arguments_phase_1(original_args):
    ...
    if len(positionals[1:]) > 2:
        warnings.warn("Giving multiple files on the command line will be deprecated, please use [bumpversion:file:...] "
                      "in a config file.", PendingDeprecationWarning,)

    root_parser = argparse.ArgumentParser(add_help=False)

    root_parser.add_argument("--config-file", metavar="FILE", default=argparse.SUPPRESS, required=False,
                            help="Config file to read most of the variables from (default: .bumpversion.cfg)", )

    root_parser.add_argument( "--verbose", action="count", default=0, help="Print verbose logging to stderr", required=False,)

    root_parser.add_argument("--list", action="store_true", default=False, help="List machine readable information", required=False, )

    root_parser.add_argument("--allow-dirty", action="store_true", default=False,
        ...
        if section_type.get("part"):
            ...
            if "serialize" in section_config:
                section_config["serialize"] = list(filter(None, (x.strip().replace("\\n", "\n") for x in
                                                                 section_config["serialize"].splitlines()),))
...
def _parse_arguments_phase_3(remaining_argv, positionals, defaults, parser2):
    parser3 = argparse.ArgumentParser(prog="bumpversion", description=DESCRIPTION,
                                      formatter_class=argparse.ArgumentDefaultsHelpFormatter, conflict_handler="resolve", parents=[parser2], )

    parser3.set_defaults(**defaults)

    parser3.add_argument("--current-version", metavar="VERSION", help="Version that needs to be updated",
                         required="current_version" not in defaults, )

    parser3.add_argument("--no-configured-files", action="store_true", default=False, dest="no_configured_files",
        help="Only replace the version in files specified on the command line, ignoring the files from the configuration file.", )

    parser3.add_argument( "--dry-run", "-n", action="store_true", default=False, help="Don't write any files, just pretend.", )

    parser3.add_argument("--new-version", metavar="VERSION", help="New version that should be in the files",
                         required="new_version" not in defaults, )

    commitgroup = parser3.add_mutually_exclusive_group()

    commitgroup.add_argument("--commit", action="store_true", dest="commit", help="Commit to version control",
                             default=defaults.get("commit", False),)
    commitgroup.add_argument("--no-commit", action="store_false", dest="commit", help="Do not commit to version control",
    ...
    taggroup.add_argument("--no-tag", action="store_false", dest="tag", help="Do not create a tag in version control",
                            default=argparse.SUPPRESS, )
