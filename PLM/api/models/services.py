...
import attr, requests, uritemplate
...
class GitHub(object):
    ...
    def pull_request(self, pr_num):
        pull_request_api_url = uritemplate.expand(self.ISSUE_ENDPOINT, dict(owner=self.owner, repo=self.repo,
                                                                            number=pr_num))

        return requests.get(pull_request_api_url, headers=self.headers).json()

    def labels(self):
        labels_api_url = uritemplate.expand(self.LABELS_ENDPOINT, dict(owner=self.owner, repo=self.repo))
    ...
    def create_release(self, release, uploads=None):
        ...
        releases_api_url = uritemplate.expand(self.RELEASES_ENDPOINT, dict(owner=self.owner, repo=self.repo))
