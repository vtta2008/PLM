...
class EventsManager(Singleton):
    ...
    Notes:
        Not supposed to be an EventHandler class instead it provides a simple
        interface to mange external events.
        Goals: - Avoid double registration of events and provide a simple interface for event
    ...
    def add_event(self, event_name, adder, adder_args=(), adder_kwargs={}, owner=None, description="", remover=None, remover_args=(), remover_kwargs={}):
        ...
        if event_name in self.data:
            logger.warning('Event name already exits. Skipped adding event. Use the override flag to override the event.')
            return
        else:
            ...
            try:
                ...
                logger.error('Failed to register callback.', exc_info=True)
                return
            ...
            logger.info('Added event named %s' % event_name)
    ...
    def remove_event(self, event_name, restore_on_fail=False):
        ...
        try:
            ...
            logger.error("Not able to remove event '{0}'".format(event_name), exc_info=True)
            return

        self._remove_from(data_copy, event_name)
        self.data = data_copy
        logger.info("Removed event '{}'".format(event_name))
    ...
    def pause_event(self, event_name):
        ...
        try:
            ...
            logger.error("Not able to pause event '{}'".format(event_name), exc_info=True)
            return

        self._toggle_paused_state(event_name)
        logger.info("Paused event '{0}'".format(event_name))
    ...
    def resume_event(self, event_name):
        ...
        if event_data:
            ...
            self._toggle_paused_state(event_name)
            logger.info("Resumed event '{0}'".format(event_name))
    ...
    def _get_event_data(self, event_name):
        ...
        if event_name in self.data.keys():
            ...
            logger.warning("Event '{0}' doesn't exist.".format(event_name))
            return {}
    ...
    def _get_event_remover(self, event_name):
        ...
        if event_data:
            try:
                ...
                logger.error("Missing event remover information for event '{}'".format(event_name),
                          exc_info=True)
        return None, (), {}
    ...
    def _remove_from(dictionary, key):
        ...
        try:
            ...
            logger.error('Not able to remove key {0} from dictionary {0}'.format(dictionary, key), exc_info=True)
