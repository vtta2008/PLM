...
# PyQt5
from PyQt5.QtCore import QTimer, Qt, QSettings, QEvent, QSize, QPointF
from PyQt5.QtWidgets import (QApplication, QWidget, QUndoStack, QAction, QUndoView, QStyleFactory, QFileDialog, QMenu,
                             QDockWidget, QMessageBox, QInputDialog)
from PyQt5.QtGui import QIcon, QKeySequence, QDoubleValidator, QCursor, QColor, QPixmap
...
from ui import options, settings, models, attributes, graphics
...
class SceneGraphUI(form_class, base_class):
    def __init__(self, parent=None, **kwargs):
        ...
        self.qsettings = settings.Settings(self.settings_file, QSettings.IniFormat, parent=self)

        self.qsettings.setFallbacksEnabled(False)

        # icon
        self.setWindowIcon(QIcon(os.path.join(options.SCENEGRAPH_ICON_PATH, 'graph_icon.png')))
    ...
    def initializeUI(self):
        ...
        self.scene_posx.setValidator(QDoubleValidator(-5000, 10000, 2, self.scene_posx))
        self.scene_posy.setValidator(QDoubleValidator(-5000, 10000, 2, self.scene_posy))
        self.view_posx.setValidator(QDoubleValidator(-5000, 10000, 2, self.view_posx))
        self.view_posy.setValidator(QDoubleValidator(-5000, 10000, 2, self.view_posy))

        self.autosave_time_edit.setValidator(QDoubleValidator(0, 1000, 2, self.autosave_time_edit))
            ...
            for filename in reversed(recent_files):
                if filename:
                    if i < self.qsettings._max_files:
                        file_action = QAction(filename, self.menu_recent_files)
    ...
    def initializePreferencesPane(self, **kwargs):
        ...
        # logging level
        log_level_str = [x[0] for x in options.LOGGING_LEVELS.items() if x[1] == logger.level][0]
        ...
        # undo viewer
        self.undoView = QUndoView(self.tab_undo)
        ...
        app_styles.extend(QStyleFactory.keys())
    ...
    def saveGraphAs(self, filename=None):
        ...
        if not filename:
            ...
            scenefile, filters = QFileDialog.getSaveFileName(self, "Save graph file",
                                                                   filename,
                                                                   "JSON files (*.json)")
    ...
    def readGraph(self, filename=None):
        ...
        if not os.path.exists(filename):
            logger.error('filename %s does not exist' % filename)
            return
        ...
        logger.debug('adding recent file: "%s"' % filename)
    ...
    def revertGraph(self):
        ...
        if filename:
            ...
            logger.info('reverting graph: %s' % filename)
            self.clearUndoStack()
    ...
    def toggleEffectsRendering(self, val):
        ...
        self.render_fx = val
        logger.info('toggling effects %s' % ('on' if val else 'off'))
    ...
    def toggleLoggingLevel(self):
        logger.level = self.logging_level_menu.itemData(self.logging_level_menu.currentIndex())
        print('# DEBUG: toggling log level: ', logger.level)
    ...
    def setAutosaveDelay(self):
        ...
        logger.info('updating autosave delay to: %d seconds.' % astime)
    ...
    def applicationStyleChanged(self, index):
        ...
        app = QApplication.instance()
        logger.debug('setting application style: "%s"' % str(style_name))
    ...
    def getAttributeEditorWidget(self):
        return self.attrEditorWidget.findChild(QWidget, 'AttributeEditor')
    ...
    def createTabMenu(self, parent):
        ...
        """
        tab_menu = QMenu(parent)
        tab_menu.clear()
        add_menu = QMenu('Add node:')
        ...
        for node in self.graph.node_types():
            node_action = QAction(node, parent)
    ...
    def readSettings(self, **kwargs):
        ...
        # font/stylesheet preferences
        for attr in ['font_family_ui', 'font_family_mono', 'font_family_nodes', 'font_size_ui', 'font_size_mono',
                     'stylesheet_name', 'palette_style', 'font_style']:
        ...
        logger.level = int(logging_level)
    ...
    def writeSettings(self):
        ...
        for attr in options.SCENEGRAPH_PREFERENCES:
            if attr == 'logging_level':
                ...
                if not hasattr(self, attr):
                    logger.debug('SceneGraphUI has no attribute "%s", skipping...' % attr)
                    continue
        ...
        # font/stylesheet preferences
        for fattr in ['font_family_ui', 'font_family_mono', 'font_family_nodes', 'font_size_ui', 'font_size_mono',
                      'stylesheet_name']:
    ...
    def formatOutputHtml(self, data, highlight=[]):
        ...
        for line in rawdata.split('\n'):
            if line:
                ind = "&nbsp;" * line.count(" ")
                fline = "<br>{0}{1}</br>".format(ind, line)
                for hname in highlight:
                    if '"%s":' % hname in line:
                        fline = '<br><b><font color="#628fab">{0}{1}</font></b></br>'.format(ind, line)
                html_result += fline
        return html_result
    ...
    def updateConsole(self, status):
        ...
        scene_str = '{0}, {1}'.format(status.get('scene_size')[0], status.get('scene_size')[1])
        self.sceneRectLineEdit.setText(scene_str)

        view_str = '{0}, {1}'.format(status.get('view_size')[0], status.get('view_size')[1])
    ...
    def promptDialog(self, label, msg):
        result = QMessageBox.question(self, label, msg, QMessageBox.Yes | QMessageBox.No)
        if (result == QMessageBox.Yes):
            return True
        return False

    def saveDialog(self, force=True):
        filename, filters = QFileDialog.getSaveFileName(self, caption='Save Current Scene', directory=os.getcwd(),
    ...
    def openDialog(self, msg, path=None):
        ...
        filename, ok = QFileDialog.getOpenFileName(self, msg, path, "JSON files (*.json)")
        if filename == "":
            return
        return filename

    def inputDialog(self, msg, label, clean=True):
        text, ok = QInputDialog.getText(self, msg, label)
        if text:
            if clean:
...
# -------------------------------------------------------------------------------------------------------------
# Created by panda on 16/06/2018 - 5:52 AM
