...
# -------------------------------------------------------------------------------------------------------------
import maya.cmds as cmds # Maya Python command
import maya.mel as mel # Maya mel command in Python
...
def findModulePath():
    ...
    if os.path.exists( scrData ):
        with open( scrData, 'r' ) as f:
            scrPth = json.load( f )
        for section in scrPth:
            if not scrPth[ section ] in sys.path:
                sys.path.append( scrPth[ section ] )
                # print "%s: (%s) has been applied into system path" % (section, scrPth[ section ])
            else:
                pass

findPath = findModulePath()

# -------------------------------------------------------------------------------------------------------------
# IMPORT QT MODULES
# -------------------------------------------------------------------------------------------------------------
from plugins import Qt # plugin module go with DAMGtool to make UI
from Qt import QtWidgets, QtCore, QtGui

# -------------------------------------------------------------------------------------------------------------
# IMPORT TOOL BOX II FUNCTIONS MODULES
# -------------------------------------------------------------------------------------------------------------
import DAMGfunctionsToolBoxII
reload( DAMGfunctionsToolBoxII )
...
def getDock(name='DAMGtoolBoxIIDock'):
    """
    ...
    ctrl = cmds.workspaceControl(name, label='DAMG Tool Box II - All About Controller')
...
def deleteDock(name='DAMGtoolBoxIIdock'):
    """
...
# ------------------------------------------------------
# User script directory, it is nomarlly in ../maya(version)/scripts
USERLIBDIR = cmds.internalVar( usd=True ) + 'DAMGpipelineTool/userLibrary/'

# Set the path of user's controller library, all the data from user library will be stored hre
DIRECTORY = os.path.join( USERLIBDIR, 'controllerLibrary' )

# ID of channel box tool (not the main one of Maya)
CHANNELBOX_ID = 'DAMGchannelBox'
...
class ControllerManager( QtWidgets.QWidget ):
    ...
    with open( scrData, 'r' ) as f:
        scrPth = json.load( f )
    scrIcons = scrPth[ 'icons_toolBox2' ]
...
class DAMGtoolBoxII( QtWidgets.QWidget ):
    ...
    nurbsType2D = {
        ...
        'Circle Arrow 2D': 'twoDcircleArrowicon.png', 'Slider Control': "sliderControl.icon.png",
    ...
    nurbsType3D = {
        ...
        'Locator Control': 'locatorControl.icon.png', 'Sphere Square': 'DAMGsphereSquareicon.PNG',
    ...
    def buildUI(self):
        ...
        mid3L = [0, 'Text','Fonts','CurveText', 'MIRROR', 'ADD ATTRIBUTE','Long Name', 'Shot Name', 'Extra Functions',
                 'TEXT CURVE', 'MIRROR']
        ...
        for x in range((len(nurbs2Dnames)/z)+1):
            for y in range(z):
                if len(count2D) >= len(nurbs2Dnames):
                    ...
                    iconPth = self.scrIcons + self.nurbsType2D[ nurbsType ]
        ...
        for x in range((len(nurbs3Dnames)/z)+1):
            for y in range(z):
                if len(count3D) >= len(nurbs3Dnames):
                    ...
                    iconPth = self.scrIcons + self.nurbsType3D[ nurbsType ]
                    icon = QtGui.QIcon( iconPth )
                    toolTip = "Create a new " + nurbsType
                    button = marv.RenderSetupButton( self, icon, mid1[ 'size' ][ 2 ] )
                    button.setMinimumSize(mid1['size'][2], mid1['size'][2])
    ...
    def colorPalletUI(self, mid2):
        ...
        rgb = {0: (.4,.4,.4), 16: (1,1,1), 3: (.75,.75,.75), 2: (.5,.5,.5), 1: (0,0,0), 18: (0,.7,1), 28: (0,.5,.5),
                    29: (0,.2,.5), 15: (0,.2,.7), 6: (0,0,1), 5: (0,0,0.4), 19: (0,1,.4), 14: (0,1,0), 23: (0,.7,.1),
