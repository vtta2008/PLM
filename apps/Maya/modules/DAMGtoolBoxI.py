...
class DAMGtoolBoxI( object ):
    def __init__(self):
        ...
        DAMGcommonToolMainUI = cmds.window('DAMGcommonToolMainUI', t="DAMG Tool Box I", rtf=True)
        #menu
        mlo = cmds.columnLayout()
        mlo_mnl = cmds.menuBarLayout()
        ...
        global tfSearch, tfReplace, tfPrefix, tfRename, tfSuffix, ifNumber, ifPadding
        global longNameAA, shortNameAA, minIntAA, defIntAA, maxIntAA, ForBAA
        #renamer tab
        t1 = cmds.columnLayout()
        cmds.separator(style = 'in', w=360, h=8)
        #copy and replace
        t1_ro1 = cmds.rowColumnLayout(nc=4, cw=cw2)
        makeDistanceForRowcolumn(2, 4)
        cmds.text(l="")
        cmds.text(l="Search", align='center')
        tfSearch = cmds.textField('tfSearch', tx="")
        cmds.text(l="")
        makeDistanceForRowcolumn(2, 4)
        cmds.text(l="")
        cmds.text(l="Replace", align='center')
        tfReplace = cmds.textField('tfReplace', tx="")
        cmds.text(l="")
        cmds.setParent('..')
        cmds.text(l="", h=8)
        
        t1_ro2 = cmds.rowColumnLayout(nc=5, cw=[(1,5),(2,172.5),(3,5),(4,172.5),(5,5)])
        cmds.text(l="")
...
#number and padding setting
        t1_ro3 = cmds.rowColumnLayout(nc=3, cw=[(1,200),(2,100),(3,60)])
        cmds.text(l="", w=200)
        cmds.text(l="Number", w=100)
        ifNumber = cmds.intField('ifNumber', v=0, w=60, min=0)
        cmds.setParent('..')
        
        t1_ro4 = cmds.rowColumnLayout(nc=3, cw=[(1,200),(2,100),(3,60)])
        cmds.button(l="Auto Rename By Type", align='center', c=self.autoRename)
        cmds.text(l="Padding", w=100)
        ifPadding = cmds.intField('ifPadding', v=0, w=60, min=0)
        cmds.text(l="", w=200, h=8)
        cmds.separator(style='in', w=160)
        cmds.text(l="", w=60, h=8)
        cmds.setParent('..')     
#rename with prefix and suffix, number and padding
        t1_ro5 = cmds.rowColumnLayout(nc=7, cw=cw3)
        cmds.text(l="", w=5)
        ...
        t1_ro6 = cmds.rowColumnLayout(nc=7, cw=cw3)
        cmds.text(l="", w=5)
        tfPrefix = cmds.textField('tfPrefix', tx="", w=105)
        cmds.text(l="", w=5)
        tfRename = cmds.textField('tfRename', tx="", w=130)
        cmds.text(l="", w=5)
        tfSuffix = cmds.textField('tfSuffix', tx="", w=105)
        cmds.text(l="", w=5)
        cmds.setParent('..')
        cmds.text(l="", h=2)
        
        t1_ro7 = cmds.rowColumnLayout(nc=7, cw=cw3)
        cmds.text(l="", w=5)
        button_prefix = cmds.button('button_prefix', l="Add Prefix", w=105, c=self.addPrefix)
        cmds.text(l="", w=5)
        button_rename = cmds.button('button_rename', l="Rename", w=130, c=self.doRename)
        cmds.text(l="", w=5)
        button_suffix = cmds.button('button_suffix', l="Add Suffix", w=105, c=self.addSuffix)
        cmds.text(l="", w=5)        
        cmds.setParent('..')
        cmds.text(l="", h=2)
        cmds.separator(style="in", w=360, h=8)
...
#long name and short name
        t2_ro1 = cmds.rowColumnLayout(nc=4, cw=[(1,30),(2,100),(3,200),(4,30)])
        cmds.text(l="")
        cmds.text(l="Long Name  ", align='center')        
        longNameAA = cmds.textField('longNameAA', text="")
        makeDistanceForRowcolumn(4, 2)
        cmds.text(l="Short Name  ", align='center')
        shortNameAA = cmds.textField('shortNameAA', text="")
        cmds.text(l="")
        cmds.setParent('..')
        cmds.text(l="", h=5)
...
#value setting: min, max, default, float or boolean
        t2_ro2 = cmds.rowColumnLayout(nc=4, cw=[(1,90),(2,90),(3,90),(4,90)])
        cmds.text(l="Min")
        cmds.text(l="Default")
        cmds.text(l="Max")
        cmds.text(l="F/B")        
        makeDistanceForRowcolumn(5, 4)
        minIntAA = cmds.intField('minIntAA', v=-360)
        defIntAA = cmds.intField('defIntAA', v=0)
        maxIntAA = cmds.intField('maxIntAA', v=360)
        ForBAA = cmds.optionMenu('ForBAA')
...
#add attribute button        
        t2_ro3 = cmds.rowColumnLayout(nc=3, cw=[(1,100),(2,160),(3,100)])
        cmds.text(l="")
        ...
        cmds.separator(style="in", w=360, h=5)
#attribute presets        
        t2_ro4 = cmds.rowColumnLayout(nc=5, cw=[(1,30),(2,147.5),(3,5),(4,147.5),(5,30)])
