...
import options
from core import log, PluginManager, Attribute, EventHandler
...
class Graph(object):
    ...
    def pasteNodes(self, nodes, offset=[200, 200]):
        ...
        for node in nodes:
            ...
            data.initialize(pos=[node.pos[0] + offset[0], node.pos[1] + offset[1]])
            data.initialize(name=new_name)
            data.pop('node_type')
            data.pop('id')
