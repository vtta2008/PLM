...
from damgteam.exceptions import SystemConfused
...
def python_2_non_unicode_repr(cls):
    ...
    A class decorator that ensures ``__repr__`` returns non-unicode when running under Python 2.
...
def python_2_non_unicode_str(cls):
    ...
    Under Python 3 it does nothing. To support Python 2 and 3 with a single code base, define a ``__str__``
...
try:
    ...
    from inspect import formatargspec, getargspec as _getargspec
