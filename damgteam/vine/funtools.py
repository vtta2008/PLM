...
# DAMG
from bin.dependencies.vine.abstract import Thenable, Promise


__all__ = ['maybe_promise', 'ensure_promise', 'ppartial', 'preplace', 'ready_promise', 'starpromise', 'transform', 'wrap', ]
...
def ensure_promise(p):
    ...
    Ensure p is a promise. If p is not a promise, a new promise is created with p' as callback.
...
def preplace(p, *args, **kwargs):
    ...
    Replace promise arguments. This will force the promise to disregard any arguments the promise is fulfilled with,
    and to be called with the provided arguments instead.
...
def transform(filter_, callback, *filter_args, **filter_kwargs):
    """
    Filter final argument to a promise. E.g. to coerce callback argument to :class:`int`:: transform(int, callback)
    or a more complex example extracting something from a dict and coercing the value to :class:`float`:
...
# -------------------------------------------------------------------------------------------------------------
# Created by panda on 8/09/2018 - 5:52 PM
