...
# DAMG
from damgteam.vine.five import with_metaclass, python_2_unicode_compatible, reraise

# -------------------------------------------------------------------------------------------------------------
__all__ = ['Thenable', 'Promise']


@with_metaclass(abc.ABCMeta)
class Thenable(Callable):
...
class Promise(object):
    ...
    def __init__(self, fun=None, args=None, kwargs=None, callback=None, on_error=None, weak=False):
    ...
    def __repr__(self):
        return ('<{0} --> {1!r}>' if self.fun else '<{0}>').format('{0}@0x{1:x}'.format(type(self).__name__, id(self)), self.fun, )
