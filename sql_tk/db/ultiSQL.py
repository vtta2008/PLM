...
backupPth = os.path.join(os.getenv('PIPELINE_TOOL'), 'sql_tk/_bk/table.map.yml')
...
#unix TEXT, username TEXT, rememberLogin INT, title TEXT, lastname TEXT,firstname TEXT, avatar TEXT, time_stamp TEXT, date_stamp TEXT
# -------------------------------------------------------------------------------------------------------------
...
def query_current_user():
    ...
    data = [str(p) for p in c.fetchall()[0]]
    return data
