...
import os, sys, logging, datetime, json, time
...
class MayaMainUI(QtWidgets.QWidget):
    ...
    def groupModeVar(self):
        ...
        self.assetsPth = self.curPth + 'scenes/_assets/'
        ...
        if self.assetsList == []:
            ...
            task = ['art', 'plt_model', 'rigging', 'surfacing']
        ...
        if self.sequencesList == []:
            ...
            self.sequencesTaskList = ['lighting', 'FX', 'layout', 'plt_anim', 'comp']
    ...
    def studioModeVar(self):
        if '_assets' in self.curPthParts:
            self.stageIndex = self.curPthParts.index('_assets')
        ...
        self.projPthParts = self.projPth.split('/')
        if self.curPthParts[self.stageIndex] == '_assets':
            ...
            self.assetsPth = self.prodPth + self.prodList[self.prodList.index('_assets')] + '/'
        self.assetsList = [f for f in os.listdir(self.assetsPth) if os.path.isdir(self.assetsPth + f)]
    ...
    def buildUI(self):
        ...
        self.bts.makeAcoolButton("Demo buttons", 'plt_rig', self.bts.waitforupdate)
        cmds.setParent(self.tabControls)
    ...
    def animationTab(self):
        ...
        self.bts.makeAcoolButton("plt_anim Tweener", 'Tweener', self.bts.tweenerUI)
    ...
    def projectContentUI(self, *args):
        ...
        assetsTab = cmds.textScrollList('assetsSelectList', ams=False, a=self.assetsTaskList,
                                        sc=self.updateSelectionTask, h=scrollListH)
        ...
        shotsTab = cmds.textScrollList('sequencesTaskList', ams=False, a=self.sequencesTaskList,
                                       sc=self.updateSequenceDetailTask, h=scrollListH)
    ...
    def commonSectionUI(self, *args):
        ...
        ccs = [self.bts.setDisplay, self.bts.setDisplay, self.bts.setDisplay, self.bts.setDisplay,
               self.bts.setDisplay, ]

        nc = 2
        commonBtns = cmds.rowColumnLayout('commonSection2', nc=nc, cw=self.bts.cwE(nc=nc, w=w, adj=adj))

        ann1 = ["Freeze Transformation", "Delete History", "Center Pivot",
                'Create transform node in center pivot of current selected']
        ...
        ann2 = ['reverse Normal', 'Display normal surface', "Select all children in hierarchy selection",
                "Create one vray displacement node", "Create multiple vray displacement node"]
        label2 = ['Rev.Normal', 'Dis.Normal', "Sel.Children", "Vray Disp", "Mul.Disp"]
        command2 = [self.bts.reverseNormal, self.bts.normalOnOff, 'cmds.select(hi=True)', self.bts.createSingleDispNode,
                    self.bts.createMultiDispNode]
        ...
        cmds.tabLayout(commonLeftTabLayout, edit=True,
                       tabLabel=((mayaWindowTab, 'Panels'), (commonMidTab, 'P.Blast'), (commonRightTab, 'Template')))
    ...
    def goToAssetsFolder(self, *args):
        ...
        if not os.path.exists(updatePth):
            message = "you dont have any _assets..."
            ...
            if cmds.tabLayout('detailTabControl', q=True, sti=True) == 1:
                ...
                if snapShotItem == []:
                    ...
                    if self.curMode == 'Studio Mode':
                        ...
                        self.updateCommentPth = self.updatePth + 'work/maya/scenes/snapShot/' + snapShotItem[
                            0] + '.comment'
                    elif self.curMode == 'Group Mode':
    ...
    def updateInfoFile(self, filePth=None, *args):
        if os.path.exists(filePth):
            ...
            lastime = datetime.datetime.strptime(time.ctime(os.path.getctime(os.path.join(filePth))),
                                                 "%a %b %d %H:%M:%S %Y")
